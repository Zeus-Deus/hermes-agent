...
| `/worktree [new [name]\|list\|prune [--dry-run]]` | Inspect or create isolated Git worktrees from the current session in CLI or Desktop. Bare `/worktree` shows the active checkout; `list` stays scoped to the session's repository; `new` creates under `.worktrees/` and moves that same session's terminal/file tools into it; `prune` uses the conservative reclaim policy. Desktop routes this through a session-scoped gateway operation, never the gateway process directory, and protects every live Desktop session's checkout while pruning. See [Git Worktrees](/user-guide/git-worktrees). |
