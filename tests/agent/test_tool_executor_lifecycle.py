...
def _instrument(agent, observed: _Observed, *, handler=None, plugin_block=None):
    ...
    def _post_hook(**kwargs):
        # The production observer returns before dispatch while the sequential
        # executor's inner registry call is suppressed. A direct mock replaces
        # that guard too, so mirror it here and count only observable events.
        from model_tools import _post_tool_call_hook_suppressed
    ...
    patches = (
        patch(
            ...
            side_effect=lambda name, args, **kwargs: (_plugin(name, args, **kwargs), None),
        ),
