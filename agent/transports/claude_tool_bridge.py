...
def build_hermes_sdk_mcp_server(
    ...
    *,
    ...
    # claude-agent-sdk 0.2.x still builds its in-process server with the MCP 1.x
    # decorator API. Hermes owns MCP 2.0, whose low-level Server takes explicit
    # callbacks instead. Keep the SDK's public config shape while adapting only
    # this in-process boundary; no SDK MCP transport is exposed to Hermes.
    ...
    handler_map = {entry.name: entry for entry in handlers}
    tools = [
        Tool.model_validate(
            {
                "name": entry.name,
    ...
    async def _call_tool(_context, params):
        entry = handler_map.get(params.name)
        if entry is None:
        ...
        response = await entry.handler(params.arguments or {})
        return CallToolResult.model_validate(
            {
    ...
    return McpSdkServerConfig(type="sdk", name=server_name, instance=server)
