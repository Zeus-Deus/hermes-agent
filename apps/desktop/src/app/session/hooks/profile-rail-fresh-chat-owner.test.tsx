...
//   active registry source    = `local` (This device)
...
// is the registry entry local::omar (scope `conn:local::omar`) — the socket
// whose WebSocket mints the runtime. Before the fix the create rode that
// socket ambiently, but the durable owner degraded to the bare string "omar":
// the optimistic row was stamped from the ambient profile with no
...
/** The registry entry local::omar answers; every other socket is a backend
 *  that never held the runtime, exactly as in the field. */
function answer(socket: MockGateway, method: string, params: Record<string, unknown>) {
  const isOmar = socket.connectUrl?.includes(`:${OMAR_PORT}`) ?? false
  ...
  if (method === 'session.create') {
    ...
    return { info: {}, session_id: RUNTIME_ID, stored_session_id: STORED_ID }
  }
...
function installDesktop(): void {
  ;(window as unknown as { hermesDesktop: unknown }).hermesDesktop = {
    ...
    getConnectionFor: vi.fn(async ({ connectionId, profile }: { connectionId: string; profile: string }) => {
      const port = connectionId === 'local' ? (profile === 'omar' ? OMAR_PORT : LOCAL_DEFAULT_PORT) : 9999
...
function Harness({
  ...
  useEffect(() => {
    onReady({
      busyRef,
      bindings: () => ({
        runtimeForStored: cache.runtimeIdByStoredSessionIdRef.current.get(STORED_ID) ?? null,
        storedForRuntime: cache.sessionStateByRuntimeIdRef.current.get(RUNTIME_ID)?.storedSessionId ?? null
      }),
...
const omarScope = registryBackendScopeKey('local', 'omar')
...
describe('profile rail: a fresh Omar chat keeps its exact registry owner across turns (#94071)', () => {
  ...
  async function bootProfileRailOmar() {
    ...
    // the state a connection-rail click leaves the window in.
    ...
    // The socket the registry dialed for local::omar (mocked HermesGateway
  ...
  it('session.create and both prompt.submit calls ride the SAME conn:local::omar socket', async () => {
    ...
    // Nothing session-scoped reached the remote primary, the local default
