...
import { type SessionOwnerRoute, sessionOwnerRouteFromRow, type SessionOwnerScope } from './session-request-router'
...
/**
 * The owner a session ROW names: the EXACT route when the row is
 ...
 * across a refresh), else the bare profile ladder of knownSessionProfile. The
 * exact rung is what lets a session's owner be reconstructed after the
 * bounded, transient hint map has evicted it or the app relaunched.
 */
export function knownSessionOwner(sessions: readonly SessionInfo[], sessionId: null | string): SessionOwnerScope {
  ...
  return sessionOwnerRouteFromRow(row) ?? knownSessionProfile(sessions, sessionId)
}
