...
/** True when `owner` names a backend. Registry topology requires an exact
 *  connection route; a bare profile is sufficient only in legacy profile-only
 *  topology, including legacy multi-profile pools. */
export function sessionOwnerIsKnown(owner: SessionOwnerScope): boolean {
  ...
  return !hasRegistryTopology() && owner != null && Boolean(String(owner).trim())
}
