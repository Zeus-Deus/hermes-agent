...
ALLOWLIST_REASONS: dict[str, str] = {
    ...
    "hermes_cli/inventory.py": (
        "TODO(legacy-retirement): remove with the direct-OAuth provider inventory path. "
        "It detects an existing Claude Code login only to keep the authenticated-provider "
        "inventory from hiding a provider the user explicitly configured; inference still "
        "routes through claude-agent-sdk."
    ),
    "hermes_cli/setup.py": (
        "TODO(legacy-retirement): remove when the direct-OAuth path is deleted (after subscription enablement; see decision record §3). "
