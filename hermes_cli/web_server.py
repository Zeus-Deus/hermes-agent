...
def _claude_code_only_status() -> Dict[str, Any]:
    """Surface the official CLI's login state without reading its secrets."""
    try:
        from hermes_cli.auth import get_external_process_provider_status

        status = dict(get_external_process_provider_status("claude-code") or {})
    except Exception as exc:
        status = {"logged_in": False, "subscription": False, "error": str(exc)}
    status["token_preview"] = None
    status["expires_at"] = None
    status["has_refresh_token"] = False
    # This card represents subscription billing specifically. A CLI logged in
    # with an API key is valid for Claude Code itself but must not make Hermes
    # claim that the user's subscription is connected.
    status["logged_in"] = bool(status.get("logged_in") and status.get("subscription"))
    return status
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    Returns None for providers we can't safely clear (the GUI shows a manual
    hint). Never delete another CLI's credential files directly.
    """
